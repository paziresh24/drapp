...
const GetCenter = ({ sendPosition, lanLng }: GetCenterProps) => {
...
const Map = ({ lat = 0, lng = 0, sendPosition }: MapProps) => {
    return (
        <MapContainer
            id={styles['map']}
            center={[lat ?? 35.70069003610754, lng]}
            zoom={20}
            scrollWheelZoom
        >
            <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
            <img src={marker} alt="" className={styles['map-marker-centered']} />
            <GetCenter sendPosition={sendPosition} lanLng={[lat, lng]} />
        </MapContainer>
    );
};
