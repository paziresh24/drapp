describe('drapp', () => {
    ...
    it('should fail salamat login and return "Salamat-Unauthorized" error', async () => {
        cy.getByTestId('change-login-with-salamat').click();
        cy.getByTestId('salamat-login-username-field').type('testUsername');
        cy.getByTestId('salamat-login-password-field').type('testPass');
        cy.getByTestId('salamat-login-login-button').click();
        cy.intercept('POST', '/V1/salamat-auth/login', {
            fixture: 'salamatUnauthorized',
            statusCode: 401
        }).as('login');
        cy.wait('@login').then(xhr => {
            console.log(xhr);
            expect(window.localStorage.getItem('token')).to.eq(null);
            expect(xhr.response.body.message).to.eq('Salamat-Unauthorized');
        });
    });
    ...
    it('should fail create center and return error: duplicated', async () => {
        cy.intercept('POST', '/V1/doctor/center/ignoreShahkar', {
            fixture: 'fieldCreateCenter',
            statusCode: 401
        }).as('createCenter');
        ...
        cy.wait('@createCenter').then(xhr => {
            expect(window.localStorage.getItem('token')).to.eq(null);
            expect(xhr.response.body.message).to.eq('duplicated');
        });
    });
});
